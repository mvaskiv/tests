...
t_list	*ft_newl(t_list *a)
{
	t_list *out;
	out = (t_list*)malloc(sizeof(t_list));
	out = ft_lstnew(ft_strmap(a->content, ft_strcap), sizeof(a->content));
	return (out);
}

void	ft_dellist(void *a, size_t n)
{
	if (a && n)
		free(a);
...
void	ft_test_lst()
{
	...
	line = "test string";
	line2 = "WorLD";
	a = ft_lstnew(line, sizeof(char) * 12);
	if (strcmp(a->content, "test string") == 0)
		puts("OK!lstnew");
	else
		puts("DIFFlstnew");
	ft_lstdelone(&a, del);
	if (a == NULL)
		...
		puts("DIFFlstdelone");
	a = ft_lstnew(line, sizeof(line));
	list = &a;
	b = ft_lstnew(line2, sizeof(line2));
	ft_lstadd(list, b);
...
//	printf("%s\n", ((*list)->next)->content);
//	c = (*list)->next
	if (strcmp((*list)->next->content, "test string") == 0)
		puts("OK!lstadd 2");
	else
		puts("DIFFlstadd 2");
	c = *list;
	ft_lstiter(c, ft_liter);
//	write(1, " T ", 3);
	puts("\nTESTING LISTMAP...\n");
	d = ft_lstmap(c, ft_newl);
	ft_lstiter(d, ft_liter);
	ft_lstdel(&d, ft_dellist);
	if (d == NULL)
	   puts("OK!lstdel");
	else
		puts("DIFFlstdel");	
